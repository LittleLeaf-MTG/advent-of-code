    ...
    while still_mutating:
        ...
        for y in range(len(todo_matrix)):
            for x in range(len(todo_matrix[0])):
                ...
                for dy in (-1, 0, 1):
                    for dx in (-1, 0, 1):
                        if (y + dy) not in range(len(todo_matrix)) or (x + dx) not in range(len(todo_matrix[0])) or dy == dx == 0:
                            continue
                        if seat_matrix[y + dy][x + dx] == '#':
                            occupied_count += 1
                if seat_matrix[y][x] == 'L' and occupied_count == 0:
                    todo_matrix[y][x] = '#'
                    still_mutating = True
                elif seat_matrix[y][x] == '#' and occupied_count >= 4:
                    todo_matrix[y][x] = 'L'
                    still_mutating = True
